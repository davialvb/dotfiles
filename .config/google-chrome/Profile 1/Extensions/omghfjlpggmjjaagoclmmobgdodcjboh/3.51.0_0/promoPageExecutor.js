...
(async () => {
  const [experiments] = await Promise.all([experimentsPromise, domLoaded]);
  const expvarid = [];
  {
    const variant = experiments[(production_default()).experiments.pollBanner.id];

    if (variant) {
      const letter = numberToAlphabetLetter(Number(variant)).toLowerCase();
      expvarid.push('254' + letter);
    }
  }
